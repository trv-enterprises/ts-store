...
Alerting is a separate resource from WS push connections — it is **not** configured by attaching rules to a `ws_connections` POST. Each alert is its own object (webhook, ws, or mqtt) managed under `/api/stores/{store}/alerts`. See [alerting-architecture.md](./alerting-architecture.md) for the full reference, including the unified create endpoint, the rule + dispatch fields, and `restart_policy` semantics.

The WS push connections described above deliver *every* record (optionally filtered/aggregated). Alerts fire only on rule matches. Configure one or the other, or both.
